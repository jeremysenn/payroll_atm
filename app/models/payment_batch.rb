...
#          customer.send_barcode_sms_message
          customer.send_barcode_sms_message_with_info("You've just been paid $#{self.PaymentAmt.round(2)} by #{customer.company.name}! Your current balance is $#{customer.balance.round(2)}. Get your cash from the PaymentATM. More information at #{Rails.application.routes.default_url_options[:host]}")
        end
      end
    end
  end
